...
import { Eye, Check, X, Trash2, Flag, Loader2, Sun, Moon } from "lucide-react";
import { toast } from "sonner";
import { useTheme } from "next-themes";

const statusStyles = {
  available: "bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border-emerald-300 dark:border-emerald-900/40",
  rejected: "bg-red-100 dark:bg-red-950/30 text-red-600 dark:text-red-400 border-red-300 dark:border-red-900/40",
  pending: "bg-amber-100 dark:bg-amber-950/20 text-amber-600 dark:text-amber-500 border-amber-300 dark:border-amber-900/30",
};
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 text-sm">
              {products.length === 0 ? (
                ...
                products.map((product) => (
                  <tr key={product._id} className="transition-colors hover:bg-zinc-100/60 dark:hover:bg-zinc-800/10">
                    
                    {/* Product Title & Category */}
                    <td className="py-4 px-6 max-w-[220px]">
                      <div className="flex flex-col min-w-0">
                        <span className="font-bold text-zinc-900 dark:text-zinc-100 tracking-wide break-words line-clamp-2">
                          {product.title}
                        </span>
                    ...
                    <td className="py-4 px-6 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="font-medium text-zinc-700 dark:text-zinc-300">{product.sellerInfo?.name}</span>
                        <span className="text-xs text-zinc-400 dark:text-zinc-500">{product.sellerInfo?.email}</span>
                      </div>
                    </td>

                    {/* Condition Badge */}
                    <td className="py-4 px-6 text-center whitespace-nowrap">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold bg-purple-100 dark:bg-purple-950/50 text-purple-600 dark:text-purple-400 border border-purple-300 dark:border-purple-900/40 tracking-wider">
                        {product.condition?.toUpperCase()}
                      </span>
                    </td>

                    {/* Price */}
                    <td className="py-4 px-6 text-center font-medium text-zinc-700 dark:text-zinc-300 whitespace-nowrap">
                    ...
                    <td className="py-4 px-6 text-center whitespace-nowrap">
                      {product.reportCount > 0 ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold bg-red-100 dark:bg-red-950/30 border border-red-300 dark:border-red-900/50 text-red-600 dark:text-red-400">
                    ...
                    <td className="py-4 px-6 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:border-zinc-400 dark:hover:border-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-200 transition-all"
                          title="View Product"
                        >
                        ...
                        {product.status === "pending" && (
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-emerald-100 dark:bg-emerald-950/30 border border-emerald-300 dark:border-emerald-900/50 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all"
                        ...
                        {product.status !== "rejected" && (
                          <button
                            ...
                            className="p-1.5 rounded-lg bg-red-100 dark:bg-red-950/30 border border-red-300 dark:border-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-500 hover:text-white transition-all"
